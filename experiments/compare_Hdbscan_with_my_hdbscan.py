...
plot_minimum_spanning_tree = False
plot_single_linkage_tree = False
plot_condensed_tree = False
plot_selected_clusters = False
plot_cluster_scatter_matplotlib = False
plot_cluster_scatter_sns = False
plot_my_dbscan_scatter = False

"""
Idea is here to compare for multiple datasets original HDBSCAN and my HDBSCAN implementation
"""
...
for dataset_name, each_datadict in datasets.items():
    ...
    clusterer_myhdbscan = MyHdbscan(min_points=min_cluster_size)
    ...
    clusterer_myhdbscan.fit_hdbscan(each_dataset)
    ...
    if plot_my_dbscan_scatter:
        num_colors = len(set(clusterer_myhdbscan.labels_))
        palette = plt.cm.viridis(np.linspace(0, 1, num_colors))
        cluster_colors = [palette[col] if col >= 0 else (0.5, 0.5, 0.5) for col in clusterer_myhdbscan.labels_]
        plt.scatter(each_dataset.T[0], each_dataset.T[1], c=cluster_colors)
        plt.title("My hdbscan scatterplot")
        plt.show()
    ...
    all_labels_match = all(np.equal(clusterer_myhdbscan.labels_, clusterer.labels_))
    print(f"All labels match: {all_labels_match}")