...
def matrix_has_row_duplicates(matrix):
    upper_triangle = np.triu(matrix)
    has_duplicates = False

    for idx, row in enumerate(upper_triangle):
        if len(np.unique(row)) < len(row) - idx:
            has_duplicates = True
            break
...
def _hdbscan_generic(
    ...
    **kwargs
    ...
    #Pairwise distance sind unterschiedlich zwischen punkten -> False
    #matrix_has_row_duplicates(distance_matrix)
    ...
    #Anscheinend hat ein punkt dieselbe mutual rechability distance zu mehreren punkten. MST ist nicht einzigartig!
    #matrix_has_row_duplicates(mutual_reachability_)
